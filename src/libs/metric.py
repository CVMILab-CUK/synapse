...
class SignalDice(nn.Module):
    def __init__(self, eps=1e-6, sep=False):
...
class SoftSignalDice(nn.Module):
    def __init__(self, eps=1e-6, sep=False, alpha=100):
...
def n_way_top_k_acc(pred, class_id, n_way, num_trials=40, top_k=1):
    ...
    for t in range(num_trials):
        ...
        acc = accuracy(pred_picked.unsqueeze(0).argmax(1), torch.tensor([0], device=pred.device), 'multiclass', num_classes=len(pred_picked), 
                    top_k=top_k)

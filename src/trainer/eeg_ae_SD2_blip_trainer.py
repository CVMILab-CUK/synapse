...
from models import eeg_AutoEncoder, Frozen_CLIPImage2TextEmbedder as ImageClip
from trainer.base_trainer import BaseTrainer
from trainer.utils import plot_recon_figures, save
from libs.losses import l1, l2, SignalDiceLoss, ContrastiveLoss, AutomaticWeightedLoss
...
class EEGAETrainer(BaseTrainer):
    def __init__(self, json_file, sharedFilePath):
        ...
        self.losses = {"sdsc":[], "rec":[], "sim":[],"loss":[],  "cos":[], "align":[]}
        self.accuracy = {"sdsc":[], "mse":[]}
        self.val_losses = {"sdsc":[], "rec":[], "sim":[], "loss":[],  "cos":[], "align":[]}
    ...
    def _train(self, gpu, size):
        ...
        for epoch in range(self.startEpoch, self.epochs):
            ...
            for step, data in enumerate(self.loader_train):
                ...
                eeg, image, label,ori_img = data['eeg'], data['image'], data['label'], data["ori_img"]
                eeg = eeg.to(gpu)
                ori_img = ori_img.to(gpu)
                image = image.to(gpu)
                b, c, s = eeg.size()
                ...
                self.losses["align"].append(align_loss.item())
                ...
                if self.globalStep % self.logIter == 0 and gpu ==0:
                    # LOG Print
                    strings = f"Train Step {self.globalStep} | LOSS {np.mean(self.losses['loss']):.4f} | SDSC LOSS {np.mean(self.losses['sdsc']):.4f} | RECON LOSS {np.mean(self.losses['rec']):.4f} | SIM LOSS {np.mean(self.losses['sim']):.4f} | COSINE LOSS {np.mean(self.losses['cos']):.4f} | ALIGN LOSS {np.mean(self.losses['align']):.4f}"
                    ...
                    self.summaryWriter.add_scalar("ALIGN LOSS", np.mean(self.losses['align']), self.globalStep)
                    ...
                    self.losses = {"sdsc":[], "rec":[], "loss":[], "sim":[],  "cos":[], "align":[]}
                    self.accuracy = {"sdsc":[], "mse":[]}
    ...
    def _valid(self, gpu):
        ...
        for step, data in enumerate(self.loader_valid):
            ...
            self.val_losses["align"].append(align_loss.item())
        ...
        if gpu == 0:
            # LOG Print
            strings = f"Valid Step {self.globalStep} | LOSS {np.mean(self.val_losses['loss']):.4f} | SDSC LOSS {np.mean(self.val_losses['sdsc']):.4f} | RECON LOSS {np.mean(self.val_losses['rec']):.4f} | SIM LOSS {np.mean(self.val_losses['sim']):.4f} | COSINE LOSS {np.mean(self.val_losses['cos']):.4f} | ALIGN LOSS {np.mean(self.val_losses['align']):.4f}"
            ...
            self.summaryWriter.add_scalar("VALID ALIGN LOSS", np.mean(self.val_losses['align']), self.globalStep)
            ...
            self.val_losses = {"sdsc":[], "rec":[], "loss":[], "sim":[], "cos":[], "align":[]}
            self.val_accuracy = {"sdsc":[], "mse":[]}
    ...
    def test(self, folder_name):
        ...
        self.makeDatasets(self.eeg_train_path, self.eeg_test_path, self.eeg_val_path, self.img_path, self.img_size,  ddp=False)
